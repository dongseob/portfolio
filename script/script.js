...
//이것도 스크립트javascript > jsx jqurey (extend function type)
...
$(function() {
  ...
  $(window).on('resize',function(){
	if (ww >= 480) { // 이건 480 미만일때 canvas 태그의 사이즈를 강제로 변환할려고 시도 그러나 이벤트 발생이 안됌
		$('.chart canvas').css({height:'130px',width:'130px'});
	}else{
		$('.chart canvas').css({height:'70px',width:'70px'});
	}
  });
	
  make_chart(); // 이건 차트 만드는 플러그인을 사용하는 함수

  function make_chart(){
	  if (ww >= 480) { // 480px 가로사이즈 적을떄 발생
			$('.chart').easyPieChart({ //easyPieChart >실행 안에있는건 옵션
			  size: 130,
			  barColor: '#FF3636',
			  scaleColor: false,
			  lineWidth: 10,
...
//수정본 끝
  ...
  var scrollHeight = 0;
  var thisTop = 0;
  $('.graphic img').on('click', function() {

	scrollHeight = $(window).scrollTop(); // 열렸을떄 scrollTop 체크 (var 쓰지않기. 팝업오픈했을때만사용하기위한 변수)
	var thisTop = $('.imgSection').offset().top;
	var onTop = thisTop - scrollHeight;
	console.log(onTop);
	//$(window).scrollTop(onTop);
	...
	$('.layerBG').fadeIn(400);
	// $('.imgBox').off('scroll touchmove mousewheel');
	closeLayer(scrollHeight);
  });
});

function closeLayer(scrollHeight) {
  //레이어 닫기
  var checkBoxBG = $('.layerBG');
  $('.layerBG').on('click', function(e) {
	if (checkBoxBG.has(e.target).length === 0) {
	  $("body").removeClass("bgFixed"); //overflow-hidden 해제(스크롤 해제)
	  $('body').off('scroll touchmove mousewheel');
	  $("#fullpage").css({
		"position": "relative",
		"top": "0"
	  });
	  $(window).scrollTop(scrollHeight); //현재 스크롤된값=보이는화면 
	  $('.imgBox').empty();
	  checkBoxBG.fadeOut(600);
	}
  });
